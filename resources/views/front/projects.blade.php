...
<style>
    ...
    <div class="project-banner banner text-center">
        <div class="mx-auto" style="max-width: 700px;">
            <h1 class="text-white bold">{{ $content['banner_section_heading'] ?? '' }}</h1>
        </div>
    </div>
            ...
            @foreach ($projects as $project)
                <div class="col-lg-3 mb-4">
                    <div class="position-relative">
                        <img src="{{ asset('storage/' . $project->image) }}" class="img-fluid w-100 project-image" alt="{{ $project->title }}">
                        <div class="overlay-box bg-white p-3 position-absolute">
                            <h5>{{ $project->title }}</h5>
                            <p>{{ Str::limit(strip_tags($project->description), 40) ? : '' }}</p>
                            <a href="{{ route('project.detail', $project->id) }}" class="btn btn-sm" style="color: #2980b9;">
